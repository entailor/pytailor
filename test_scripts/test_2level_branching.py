...
with DAG(name='DAG') as dag_2lvl_dup:
    ...
    # outer branching
    with BranchTask(name='DUP1', parents=t1,
                    branch_data=['<% $.files.testfiles1 %>', '<% $.inputs.data %>'],
                    branch_files=['testfiles1']) as dup1:
        ...
        with DAG(name='SUBDAG') as sub_dag:
            ...
            # inner branching
            with BranchTask(name='DUP2', parents=t2,
                            branch_data=['<% $.files.testfiles2 %>',
                                         '<% $.outputs.out_from_T1 %>',
                                         '<% $.inputs.data %>'],
                            branch_files=['testfiles2']) as dup2:
...
fileset = FileSet(prj)
fileset.upload(
    testfiles1=['testfiles/testfile_01.txt',
                'testfiles/testfile_02.txt',
                'testfiles/testfile_03.txt'],
    testfiles2=['testfiles/testfile_01.txt',
                'testfiles/testfile_02.txt',
                'testfiles/testfile_03.txt'],
)
...
target_output = {'out_from_T1': [3, 4, 5],
                 'out_from_T10': "{'0': 'newfile1.txt', '1': 'newfile1.txt', '2': "
                                 "'newfile1.txt'}",
                 ...
                 'out_from_T6': {'0': '[0, 1, 2]', '1': "['a', 'b', 'c']",
                                 '2': '[6, 7, 8]'},
                 'out_from_T7': {'0': "{'0': 'newfile2.txt', '1': 'newfile2.txt', '2': "
                                      "'newfile2.txt'}",
                                 '1': "{'0': 'newfile2.txt', '1': 'newfile2.txt', '2': "
                                      "'newfile2.txt'}",
                                 '2': "{'0': 'newfile2.txt', '1': 'newfile2.txt', '2': "
                                      "'newfile2.txt'}"},
                 'out_from_T8': {'0': 'newfile1.txt', '1': 'newfile1.txt',
                                 '2': 'newfile1.txt'},
                 'out_from_T9': "{'0': {'0': 'newfile2.txt', '1': 'newfile2.txt', '2': "
                                "'newfile2.txt'}, '1': {'0': 'newfile2.txt', '1': "
                                "'newfile2.txt', '2': 'newfile2.txt'}, '2': {'0': "
                                "'newfile2.txt', '1': 'newfile2.txt', '2': "
                                "'newfile2.txt'}}"}

assert wf.outputs == target_output
